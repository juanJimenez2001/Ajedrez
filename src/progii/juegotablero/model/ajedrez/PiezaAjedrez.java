...
@Programacion2 (
   ...
   */
  @Override
  public final boolean movimientoDentroLimites (int toFila, int toColumna) {//movimientoDentroLimites
    boolean movimiento=false;
    if(toColumna<=7 && toColumna>=0 && toFila<=7 && toFila>=0 && (toFila!=getFila() || toColumna!=getColumna()))
      movimiento=true;
    return movimiento;
  }//movimientoDentroLimites
   ...
   */
  public final boolean puedeMover (int toFila, char toColumna){//puedeMover
    boolean movimiento=false;
    if(toColumna<='h' && toColumna>='a' && toFila<=8 && toFila>=1 && (toFila!=getFilaAjedrez() || toColumna!=getColumnaAjedrez()))
      movimiento=true;
    return movimiento;
  }//puedeMover
...
// métodos auxiliares
   ...
   */
  protected void casillasVisitables (IList<Casilla> resultado,int incFila, int incColumna){//casillasVisitables
    int x=getFila();
    int y=getColumna();
    int i=0;
    boolean seguir=true;
    while(seguir==true) {
      i=resultado.size();
      casillaVisitable(resultado, x+incFila, y+incColumna);
      if (i==resultado.size() || queHay(x+incFila,y+incColumna)!=null)
        seguir=false;
      x=x+incFila;
      y=y+incColumna;
    }
